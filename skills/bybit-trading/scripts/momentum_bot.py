"""
Momentum Trading Bot - Pybit Version
Timeframe: 15m
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/bybit_bots/momentum_bot.log')
    ]
)
...
class MomentumBot:
    ...
    def get_signal(self, df):
        ...
        if (current['close'] > prev['high_max'] and 
            current['volume'] > current['volume_sma'] * VOLUME_MULTIPLIER and
            current['atr'] > df['atr'].rolling(10).mean().iloc[-1]):
            logger.info(f"Momentum LONG signal | Price: {current['close']:.2f} | Breakout above {prev['high_max']:.2f}")
            return 'Buy'
        ...
        if (current['close'] < prev['low_min'] and 
            current['volume'] > current['volume_sma'] * VOLUME_MULTIPLIER and
            current['atr'] > df['atr'].rolling(10).mean().iloc[-1]):
            logger.info(f"Momentum SHORT signal | Price: {current['close']:.2f} | Breakdown below {prev['low_min']:.2f}")
            return 'Sell'
    ...
    def place_order(self, side, qty):
        ...
        try:
            ...
            if order['retCode'] == 0:
                logger.info(f"Momentum {side} opened | Qty: {qty} | Entry: {entry:.2f} | TP: {tp:.2f} | SL: {sl:.2f}")
                return order
            else:
    ...
    def run(self):
        """Main trading loop."""
        logger.info("=" * 50)
        logger.info("Momentum Bot Started")
        logger.info(f"Symbol: {SYMBOL} | Timeframe: {TIMEFRAME}m")
        logger.info(f"TP: {TP_PCT}% | SL: {SL_PCT}% | Lookback: {LOOKBACK}")
        logger.info("=" * 50)
